...
import { db, notifications, notificationTemplates, notificationRules, users, userRoles, roles, } from "@growengine/db";
...
export async function deliverNotification(notificationId) {
    ...
    try {
        switch (record.channel) {
            ...
            case "slack": {
                const url = record.metadata.webhookUrl || env.slackWebhookUrl;
                if (!url)
            ...
            case "teams": {
                const url = record.metadata.webhookUrl || env.teamsWebhookUrl;
                if (!url)
            ...
            case "webhook": {
                const url = record.metadata.webhookUrl;
                if (!url)
                    throw new Error("No webhook URL in metadata");
                const res = await fetch(url, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
 ...
 */
export async function applyNotificationRules(tenantId, eventType, payload) {
    const rules = await db
        .select()
        .from(notificationRules)
        .where(and(eq(notificationRules.tenantId, tenantId), eq(notificationRules.eventType, eventType), eq(notificationRules.isActive, 1)));
    for (const rule of rules) {
        const [template] = await db
            .select()
            .from(notificationTemplates)
            .where(and(eq(notificationTemplates.tenantId, tenantId), eq(notificationTemplates.key, rule.templateKey)));
        ...
        const recipientIds = new Set(audience.userIds ?? []);
        if (audience.roles?.length) {
            ...
            const wantedRoleIds = new Set(tenantRoleNames.filter((r) => audience.roles.includes(r.name)).map((r) => r.id));
            const memberships = await db.select().from(userRoles);
            for (const m of memberships) {
                if (wantedRoleIds.has(m.roleId))
        ...
        for (const channel of channels) {
            if (recipientIds.size === 0 && channel !== "slack" && channel !== "teams" && channel !== "webhook")
                continue;
