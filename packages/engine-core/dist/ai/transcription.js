 ...
 */
export async function transcribeAudio(audioBuffer, fileName, ctx) {
...
async function transcribeLocal(audioBuffer, fileName, startedAt) {
    const dir = await mkdtemp(join(tmpdir(), "ge-whisper-"));
    const inputPath = join(dir, fileName);
    const outBase = join(dir, "out");
    await writeFile(inputPath, audioBuffer);
    try {
        await execFileAsync(env.whisperCppPath, ["-m", env.whisperCppModelPath, "-f", inputPath, "-oj", "-of", outBase], { maxBuffer: 64 * 1024 * 1024, timeout: 60 * 60 * 1000 });
...
async function transcribeViaApi(audioBuffer, fileName, ctx, startedAt) {
    ...
    try {
        ...
        const durationMinutes = verbose.segments && verbose.segments.length > 0
            ? verbose.segments[verbose.segments.length - 1].end / 60
            : 1;
        await recordAiCost({ ...ctx, feature: "transcription" }, "openai", "whisper-1", Math.ceil(durationMinutes * 1000), 0);
        return {
            engine: "whisper_api",
