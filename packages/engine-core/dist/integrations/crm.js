...
export const zohoCrmConnector = {
    ...
    async fetchMetrics(ctx, since, until) {
        ...
        metrics.push({ ...base, metric: "leads", value: leadCount }, { ...base, metric: "deals_won", value: dealsWon }, { ...base, metric: "deals_lost", value: dealsLost });
...
async function zohoCount(ctx, apiDomain, headers, module, dateField, since, until, requestIds, extraCriteria) {
    ...
    if (!response.ok) {
        if (response.status === 401)
            throw new ConnectorError("zoho_crm", "Access token expired", true, requestId);
        return 0;
    }
...
export const hubspotConnector = {
    ...
    async fetchMetrics(ctx, since, until) {
        ...
        const requestIds = [];
        const search = async (object, filters) => {
            const { response, requestId } = await rateLimitedFetch("hubspot", ctx.tenantId, `https://api.hubapi.com/crm/v3/objects/${object}/search`, {
                method: "POST",
                headers,
    ...
    async testConnection(ctx) {
        const { response } = await rateLimitedFetch("hubspot", ctx.tenantId, "https://api.hubapi.com/crm/v3/objects/contacts?limit=1", { headers: { Authorization: `Bearer ${ctx.credentials.accessToken}` } });
        return response.ok
...
export const salesforceConnector = {
    ...
    async fetchMetrics(ctx, since, until) {
        ...
        const soql = async (query) => {
            const { response, requestId } = await rateLimitedFetch("salesforce", ctx.tenantId, `${instance}/services/data/v62.0/query?q=${encodeURIComponent(query)}`, { headers });
            if (requestId)
                requestIds.push(requestId);
            if (!response.ok) {
                throw new ConnectorError("salesforce", `HTTP ${response.status}`, response.status !== 401, requestId);
            }
            return (await response.json()).totalSize ?? 0;
        };
        ...
        const dealsWon = await soql(`SELECT COUNT() FROM Opportunity WHERE IsWon = true AND CloseDate >= ${since} AND CloseDate <= ${until}`);
        const dealsLost = await soql(`SELECT COUNT() FROM Opportunity WHERE IsClosed = true AND IsWon = false AND CloseDate >= ${since} AND CloseDate <= ${until}`);
    ...
    async refreshToken(ctx) {
        if (!ctx.credentials.refreshToken || !ctx.credentials.clientId)
            return null;
    ...
    async testConnection(ctx) {
        const { response } = await rateLimitedFetch("salesforce", ctx.tenantId, `${ctx.credentials.instanceUrl}/services/data/v62.0/limits`, { headers: { Authorization: `Bearer ${ctx.credentials.accessToken}` } });
        return response.ok
...
export const dynamicsConnector = {
    ...
    async fetchMetrics(ctx, since, until) {
        ...
        const requestIds = [];
        const count = async (entitySet, filter) => {
            const { response, requestId } = await rateLimitedFetch("dynamics", ctx.tenantId, `${orgUrl}/api/data/v9.2/${entitySet}/$count?$filter=${encodeURIComponent(filter)}`, { headers });
            if (requestId)
                requestIds.push(requestId);
            if (!response.ok) {
    ...
    async refreshToken(ctx) {
        if (!ctx.credentials.refreshToken || !ctx.credentials.clientId || !ctx.credentials.tenantId)
            return null;
        const res = await fetch(`https://login.microsoftonline.com/${ctx.credentials.tenantId}/oauth2/v2.0/token`, {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: new URLSearchParams({
                grant_type: "refresh_token",
    ...
    async testConnection(ctx) {
        const { response } = await rateLimitedFetch("dynamics", ctx.tenantId, `${ctx.credentials.orgUrl}/api/data/v9.2/WhoAmI`, { headers: { Authorization: `Bearer ${ctx.credentials.accessToken}` } });
        return response.ok
...
export const odooConnector = {
    provider: "odoo",
    async fetchMetrics(ctx, since, until) {
        const call = async (model, domain) => {
            const { response, requestId } = await rateLimitedFetch("odoo", ctx.tenantId, `${ctx.credentials.baseUrl}/jsonrpc`, {
                method: "POST",
            ...
            const body = (await response.json());
            if (body.error)
                throw new ConnectorError("odoo", body.error.message, false, requestId);
        ...
        if (!auth.result)
            throw new ConnectorError("odoo", "Authentication failed", false);
        const callWithUid = async (model, domain) => {
            const { response, requestId } = await rateLimitedFetch("odoo", ctx.tenantId, `${ctx.credentials.baseUrl}/jsonrpc`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
