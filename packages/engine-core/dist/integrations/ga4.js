...
async function getAccessToken(ctx) {
    if (ctx.credentials.refreshToken && env.googleClientId && env.googleClientSecret) {
        ...
        if (!res.ok)
            throw new ConnectorError("ga4", `OAuth refresh failed: HTTP ${res.status}`, false);
    ...
    if (env.ga4ServiceAccountJson) {
        ...
        if (!access_token)
            throw new ConnectorError("ga4", "Service account authorization failed", false);
        return access_token;
    }
    throw new ConnectorError("ga4", "No GA4 credentials: configure GA4_SERVICE_ACCOUNT_JSON or connect via Google OAuth", false);
}
export const ga4Connector = {
    ...
    async fetchMetrics(ctx, since, until) {
        ...
        if (!response.ok || body.error) {
            throw new ConnectorError("ga4", body.error?.message ?? `HTTP ${response.status}`, body.error?.status !== "PERMISSION_DENIED", requestId);
        }
        const metrics = [];
    ...
    async testConnection(ctx) {
        try {
            const token = await getAccessToken(ctx);
            const propertyId = ctx.externalAccountId.replace(/^properties\//, "");
            const { response } = await rateLimitedFetch("ga4", ctx.tenantId, `${DATA_API}/properties/${propertyId}/metadata`, { headers: { Authorization: `Bearer ${token}` } });
            return response.ok
