...
import { chooseAi, fallbackOrder, type AiChoice, type AiProvider } from "./select.js";
...
export function providerKeys(): Record<AiProvider, boolean> {
  return {
...
function envModels(): Record<AiProvider, string> {
...
export async function resolveAi(forced?: AiProvider): Promise<AiChoice> {
...
function geminiUsage(res: { usageMetadata?: { promptTokenCount?: number; candidatesTokenCount?: number; thoughtsTokenCount?: number } }) {
...
export async function aiComplete(
  ...
  const attempts: AiChoice[] = [first];
  if (!opts.provider) {
    for (const provider of fallbackOrder(first.provider, keys)) attempts.push({ provider, model: models[provider] });
  }
...
export async function aiCompleteStructured<T>(
  ...
  const loose = await aiCompleteJson<unknown>(prompt, ctx, { ...opts, provider: "openai" });
