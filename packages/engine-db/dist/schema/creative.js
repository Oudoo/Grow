...
import { mysqlTable, text, timestamp, index, date, varchar, decimal, json, } from "drizzle-orm/mysql-core";
 ...
 */
export const catApprovals = mysqlTable("cat_approvals", {
    id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => randomUUID()),
    tenantId: varchar("tenant_id", { length: 36 })
        .notNull()
        .references(() => tenants.id, { onDelete: "cascade" }),
 ...
 */
export const abPilots = mysqlTable("ab_pilots", {
    id: varchar("id", { length: 36 }).primaryKey().$defaultFn(() => randomUUID()),
    tenantId: varchar("tenant_id", { length: 36 })
        .notNull()
        .references(() => tenants.id, { onDelete: "cascade" }),
