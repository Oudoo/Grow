...
export async function createCreativeAsset(formData: FormData) {
  ...
  const __assetId = crypto.randomUUID();
  await db
    ...
    ;
  const [asset] = await db.select().from(creativeAssets).where(eq(creativeAssets.id, __assetId));
...
export async function requestCatApproval(assetId: string) {
  ...
  const __approvalId = crypto.randomUUID();
  await db
    .insert(catApprovals)
    .values({
        id: __approvalId,
    ...
    ;
  const [approval] = await db.select().from(catApprovals).where(eq(catApprovals.id, __approvalId));
