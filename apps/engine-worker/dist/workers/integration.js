...
import { db, integrations, integrationLogs, metricRecords, usageRecords, } from "@growengine/db";
import { createPollWorker, QUEUE_NAMES, getConnector, decryptJson, encryptJson, validateIncomingMetric, detectAnomaly, summarizeFindings, publishEvent, EVENT_TYPES, } from "@growengine/core";
 ...
 */
async function buildContext(integrationId) {
...
async function handleSync(data) {
    ...
    try {
        ...
        const since = data.options?.since ??
        ...
        for (const raw of metrics) {
            ...
            if (!history) {
                const rows = await db
                    ...
                    .from(metricRecords)
                    .where(and(eq(metricRecords.integrationId, row.id), eq(metricRecords.metric, metric.metric)))
...
export function createIntegrationWorker() {
    return createPollWorker(QUEUE_NAMES.integration, async (job) => {
