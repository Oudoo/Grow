 ...
 */
export async function handleMeetingAnalysis(data) {
    ...
    if (!transcript && meeting.recordingStorageKey) {
        ...
        const result = await transcribeAudio(audio, meeting.recordingStorageKey.split("/").pop() ?? "recording.mp3", ctx);
    ...
    const analysis = await aiCompleteJson(`You are a senior account strategist. Analyze this client meeting transcript and the pre-meeting form responses.
...
- "expectationBaselineMarkdown": a complete Expectation Baseline document in markdown with sections: Context, Agreed Goals, Success Metrics, Assumptions, Out of Scope, Open Questions. Only include items actually supported by the transcript.`, ctx, { maxTokens: 8000 });
    const confidence = computeConfidence({
        evidenceCount: analysis.requirements.length + analysis.challenges.length + analysis.actionItems.length,
        dataWindowDays: 1,
...
export async function handleSowGeneration(data) {
    ...
    const sow = await aiCompleteJson(`You are a delivery director producing a Scope of Work from analyzed meeting intelligence.
...
Base every estimate on the listed requirements only.`, ctx, { maxTokens: 8000 });
    const confidence = computeConfidence({
        evidenceCount: meeting.extractedRequirements.length,
        dataWindowDays: 1,
 ...
 */
export async function handleRecommendationVerify(data) {
    const { recommendationId } = data.input;
    const [rec] = await db
        .select()
        .from(recommendations)
        .where(and(eq(recommendations.id, recommendationId), eq(recommendations.tenantId, data.tenantId)));
    if (!rec)
    ...
    const facts = await db
        ...
        .from(metricRecords)
        .where(and(eq(metricRecords.tenantId, data.tenantId), eq(metricRecords.clientId, rec.clientId), dsql `${metricRecords.date} >= CURRENT_DATE - INTERVAL '90 days'`))
    ...
    const verification = await aiCompleteJson(`You are a rigorous analytics verifier. Verify each factual claim in this recommendation against the ACTUAL metric digest below. A claim is "supported" only when the digest data backs it.
...
- "revisedBody": only when needs_revision — the recommendation rewritten to drop unsupported claims.`, ctx, { maxTokens: 4000 });
    const evidence = verification.claims.map((c) => {
        const records = c.metricsUsed.flatMap((m) => byMetric.get(m)?.sample.map((s) => ({ id: s.id, requestId: s.sourceRequestId, url: s.sourceReferenceUrl })) ?? []);
        return {
            claim: c.claim,
            verdict: c.verdict,
            evidence: c.evidence,
 ...
 */
export async function handleDmaicGeneration(data) {
    const { dmaicProjectId } = data.input;
    const [project] = await db
        .select()
        .from(dmaicProjects)
        .where(and(eq(dmaicProjects.id, dmaicProjectId), eq(dmaicProjects.tenantId, data.tenantId)));
    if (!project)
        throw new Error(`DMAIC project ${dmaicProjectId} not found`);
    const facts = await db
        .select({
        metric: metricRecords.metric,
        total: dsql `SUM(${metricRecords.value})`,
        days: dsql `COUNT(DISTINCT ${metricRecords.date})`,
        provider: dsql `MIN(${metricRecords.provider}::text)`,
    })
        .from(metricRecords)
        .where(and(eq(metricRecords.tenantId, data.tenantId), eq(metricRecords.clientId, project.clientId), dsql `${metricRecords.date} >= CURRENT_DATE - INTERVAL '90 days'`))
    ...
    const dmaic = await aiCompleteJson(`You are a Lean Six Sigma master black belt working in a growth marketing agency. Build a complete DMAIC project.
...
Ground baselines in the metric summary; never invent numbers.`, ctx, { maxTokens: 8000 });
...
export async function handleReport(data) {
    const { clientId, title, focus } = data.input;
    const facts = await db
        .select({
        metric: metricRecords.metric,
        date: metricRecords.date,
        ...
        .from(metricRecords)
        .where(and(eq(metricRecords.tenantId, data.tenantId), eq(metricRecords.clientId, clientId), dsql `${metricRecords.date} >= CURRENT_DATE - INTERVAL '30 days'`))
    ...
    const markdown = await aiComplete(`Write a client performance report in markdown titled "${title}". ${focus ? `Focus: ${focus}.` : ""}
Use ONLY this real daily data from the last 30 days (metric, date, value):
${JSON.stringify(facts.slice(0, 1500))}
Structure: Executive Summary, Performance by Metric (with concrete numbers), Notable Changes, Risks, Next Steps. Never invent numbers; if data is missing for an area, say so.`, ctx, { maxTokens: 6000 });
    ...
    try {
        await indexEntity({ tenantId: data.tenantId, clientId, entityType: "knowledge_document", entityId: doc.id, text: markdown }, ctx);
    }
    catch {
        /* embeddings optional */
    }
