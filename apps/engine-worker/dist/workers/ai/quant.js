...
import { db, metricRecords, forecasts, seasonalityPatterns, lostOpportunities, healthScores, teamScorecards, tasks, tickets, users, clients, } from "@growengine/db";
import { forecastSeries, extractMonthlySeasonality, quantifyLostOpportunity, coefficientOfVariation, computeConfidence, publishEvent, EVENT_TYPES, aiComplete, } from "@growengine/core";
/**
 ...
 */
async function loadDailySeries(tenantId, clientId, metric) {
    const rows = await db
        .select({
        date: metricRecords.date,
        value: dsql `SUM(${metricRecords.value})`,
        provider: dsql `MIN(${metricRecords.provider}::text)`,
        sanityPassed: dsql `SUM(CASE WHEN ${metricRecords.sanityStatus} = 'passed' THEN 1 ELSE 0 END)`,
        total: dsql `COUNT(*)`,
        latestFetch: dsql `MAX(${metricRecords.fetchedAt})`,
    })
        .from(metricRecords)
        .where(and(eq(metricRecords.tenantId, tenantId), eq(metricRecords.clientId, clientId), eq(metricRecords.metric, metric)))
...
export async function handleForecast(data) {
    const { clientId, metric, horizonDays, forecastId } = data.input;
    await db.update(forecasts).set({ status: "running" }).where(eq(forecasts.id, forecastId));
    const { series, sources, sanityPassRate, latestFetch } = await loadDailySeries(data.tenantId, clientId, metric);
    if (series.length === 0) {
        await db
            .update(forecasts)
...
export async function handleSeasonality(data) {
    const { clientId, metric } = data.input;
    const { series, sanityPassRate, sources, latestFetch } = await loadDailySeries(data.tenantId, clientId, metric);
    if (series.length < 60)
        return;
...
export async function handleLostOpportunity(data) {
    const { clientId, metric } = data.input;
    const { series, sanityPassRate, sources, latestFetch } = await loadDailySeries(data.tenantId, clientId, metric);
    if (series.length < 90)
        return;
...
export async function handleHealthScore(data) {
    ...
    const recent = (s, days) => s.slice(-days).reduce((a, p) => a + p.value, 0);
    const drivers = [];
    let performance = 60;
    if (spend.series.length >= 28 && conversions.series.length >= 28) {
        ...
        const cpaPrior = recent(spend.series.slice(0, -14), 14) /
    ...
    const openTasks = await db
        ...
        .from(tasks)
        .where(and(eq(tasks.tenantId, data.tenantId), eq(tasks.clientId, clientId), dsql `${tasks.status} NOT IN ('done','cancelled')`, dsql `${tasks.dueDate} < CURRENT_DATE`));
    ...
    const openTickets = await db
        .select({ count: dsql `COUNT(*)` })
        .from(tickets)
        .where(and(eq(tickets.tenantId, data.tenantId), eq(tickets.clientId, clientId), dsql `${tickets.status} IN ('open','in_progress')`));
    const engagement = Math.max(20, 95 - Number(openTickets[0]?.count ?? 0) * 5);
    const growth = revenue.series.length >= 28
        ? Math.max(0, Math.min(100, 60 +
            ((recent(revenue.series, 14) - recent(revenue.series.slice(0, -14), 14)) /
                ...
                200))
        : 60;
    const score = Math.round((performance * 0.35 + delivery * 0.25 + engagement * 0.15 + growth * 0.25) * 100) /
        100;
...
export async function handleScorecards(data) {
    const { periodStart, periodEnd } = data.input;
    ...
    for (const user of teamUsers) {
        const userTasks = await db
            .select()
            .from(tasks)
            .where(and(eq(tasks.tenantId, data.tenantId), eq(tasks.assigneeId, user.id), gte(tasks.updatedAt, new Date(periodStart))));
        const completed = userTasks.filter((t) => t.status === "done" && t.completedAt && t.completedAt <= new Date(`${periodEnd}T23:59:59Z`));
        const onTime = completed.filter((t) => !t.dueDate || (t.completedAt && t.completedAt <= new Date(`${t.dueDate}T23:59:59Z`)));
        ...
        const userTickets = await db
            .select()
            .from(tickets)
            .where(and(eq(tickets.tenantId, data.tenantId), eq(tickets.assigneeId, user.id), gte(tickets.updatedAt, new Date(periodStart))));
        ...
        if (completed.length === 0 && resolved.length === 0)
            continue;
