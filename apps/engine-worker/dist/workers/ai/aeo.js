...
import { aiCompleteJson, crawlSite, computeConfidence, publishEvent, EVENT_TYPES, } from "@growengine/core";
function extractSignals(page) {
    const jsonLdTypes = [];
    ...
    const questionHeadings = page.headings.filter((h) => /^(what|how|why|when|where|who|can|should|is|are|do|does)\b/i.test(h.text)).length;
...
export async function handleAeoAudit(data) {
    const { aeoAuditId } = data.input;
    const [auditRow] = await db
        .select()
        .from(aeoAudits)
        .where(and(eq(aeoAudits.id, aeoAuditId), eq(aeoAudits.tenantId, data.tenantId)));
    if (!auditRow)
        throw new Error(`AEO audit ${aeoAuditId} not found`);
    ...
    try {
        ...
        const schemaMarkup = pct(signals.filter((s) => s.jsonLdTypes.length + s.schemaTypes.length > 0).length, signals.length);
        const answerability = Math.min(100, (signals.reduce((a, s) => a + s.questionHeadings, 0) / Math.max(signals.length, 1)) * 25);
        const citeableQuotes = Math.min(100, (signals.reduce((a, s) => a + s.shortAnswerBlocks, 0) / Math.max(signals.length, 1)) * 10);
        const ctx = {
        ...
        const aiResult = await aiCompleteJson(`You are an AEO (Answer Engine Optimization) specialist evaluating whether content is structured for generative AI search engines (citeable quotes, direct answers, structured data, entity clarity).
...
- "contentQualityScore": 0-100 for LLM-citeability of the copy itself.`, ctx, { maxTokens: 5000 });
        ...
        const overall = Math.round((dimensionScores.structuredData * 0.25 +
            dimensionScores.schemaMarkup * 0.2 +
            dimensionScores.answerability * 0.2 +
            dimensionScores.citeableQuotes * 0.15 +
            dimensionScores.contentQuality * 0.2) * 100) / 100;
