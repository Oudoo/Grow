...
import { createPollWorker, QUEUE_NAMES, EVENT_TYPES, enqueueAiJob, enqueueNotificationJob, applyNotificationRules, notify, } from "@growengine/core";
...
export function createEventsWorker() {
    return createPollWorker(QUEUE_NAMES.events, async (job) => {
        ...
        for (const [index, handler] of (handlers[eventType] ?? []).entries()) {
            try {
                ...
                results.push({ handler: `builtin_${index}`, ok: false, error: err.message });
            }
        }
        ...
        if (tenantId) {
            try {
                ...
                results.push({ handler: "notification_rules", ok: false, error: err.message });
            }
            // 3. Outbound webhooks
            try {
                ...
                results.push({ handler: "webhooks", ok: false, error: err.message });
            }
        }
        await db
            .update(domainEvents)
